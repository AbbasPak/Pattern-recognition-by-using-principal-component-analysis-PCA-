...
---
# Analysing practical examples by using principal component analysis (PCA)
```{r, echo = FALSE}
...
Different packages are used in R for performing Principal Component Analysis (PCA) such as `stats`,  `FactoMine`, `FactoExtra`, `princurve`, …. Each package has its own set of functions and features, so you can choose the one that best suits your specific needs and preferences. 
...
```{r}
df<- data.frame(read.table("I://Data-Science-2024//PCA//data3.txt"))
```
_Performing PCA_
```{r, echo=FALSE}
df<-scale(df)
